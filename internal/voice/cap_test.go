...
func TestLintAll_statementWordCap(t *testing.T) {
	long := strings.Repeat("The landlord must act. ", 32) // 128 words, short sentences
	...
	if !found {
		t.Errorf("a 128-word statement passed the cap: %v", v)
	}
	if v := LintAll("en", map[string]string{"intro_md": long}); len(v) != 0 {
		t.Errorf("the cap applied to a non-statement text: %v", v)
	}
	if v := LintAll("en", map[string]string{"body_md": strings.Repeat("The landlord must act. ", 28)}); len(v) != 0 {
		t.Errorf("a 112-word statement failed: %v", v)
	}
}
