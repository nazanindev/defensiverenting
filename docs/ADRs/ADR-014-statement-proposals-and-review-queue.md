...
The queue page (D5) is now the list it was meant to be. One list, proposed edits first and then drift findings, each in reading order, with one number at the top: items left. One item is open at a time, and a decision opens the next instead of returning to the top. An edit shows the proposer's own account first ("Answers … Did …"), then the statement as a word diff in place, then only the citations that change. A drift finding shows the confirmed sentence and the page's sentence now with the changed words marked, and the statement under it. Two buttons: Apply and Not this, with the reason kept. "later" is a small link, since a snoozed item is how a reviewer defers on purpose without blocking publication. Kind tabs are gone; the decided lists are links at the foot. The raw evidence panel, the similarity percentage, and the two-column layout are gone with them.

**Drift findings do not supersede edits (2026-09-20).** D2's rule that a newer proposal supersedes an older one on the same key was written for competing replacements. The first check run after the rules above filed seven findings and, by that rule, knocked three unread triage edits out of the queue. A source-drift finding is the checker's report that a quote moved, not a replacement: it now supersedes only an earlier drift finding on the key. An edit filed after a finding still supersedes it, as the newer word on the statement.

## Amendment, 2026-09-20: a statute is quoted whole, because the quote is the tripwire

A citation's quote does two jobs. It is the evidence that the statement is supported, and it is the only text the checker watches for the law changing. A one-sentence quote of a subsection does the first and fails the second: an amendment to any other sentence in the provision passes unnoticed. The agent was quoting a sentence because the tool told it to ("the verbatim line"). The public site never shows the quote, so a whole-subsection quote costs the reader nothing.

The instruction now says: for a statute or regulation, quote the whole subsection the locator names, from its marker to its end; for guidance, the paragraph; a single line only for a directory entry or a figure. A statute quote under 25 words is a lint, not a rejection (`drafting.NarrowQuote`): the save goes through and a reviewer note lands on the statement's card. `triage narrow` lists the citations already on the site that fall under it, for a widening pass that files each as an `agent-pass:widen-quote` edit through the ordinary queue.
