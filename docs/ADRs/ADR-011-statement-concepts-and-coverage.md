...
### D4. National pages link down along the concept graph

Under a tagged national statement, the page renders a compact "Specifics for: Massachusetts · Pennsylvania · …" row — every place with a localized instance, deep-linked. Tagged statements render their concept slug as an HTML anchor, so the link lands on `/j/massachusetts/boston/security-deposits#retaliation-protection`, not the top of the page. This converts ADR-009's mandated dead ends into doors using one content-driven query at render time: no duplicated text, no per-reader state (contrast the cache-leak concern that shaped ADR-009 D5 — these links are the same for every reader), and internal links only, so nothing here touches how search engines see the pages.
